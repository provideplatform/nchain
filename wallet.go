...
func (w *Wallet) generate(db *gorm.DB, gpgPublicKey string) {
	...
	if network.isEthereumNetwork() {
		...
		Log.Warningf("Unable to generate private key for wallet using unsupported network: %s", w.NetworkID.String())
	}
...
func (w *Wallet) NativeCurrencyBalance() (*big.Int, error) {
	...
	if network.isEthereumNetwork() {
		...
		Log.Warningf("Unable to read native currency balance for network: %s", w.NetworkID)
	}
	return balance, nil
}
