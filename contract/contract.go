...
func (c *Contract) executeEthereumContract(network *network.Network, tx *tx.Transaction, method string, params []interface{}) (*interface{}, *interface{}, error) { // given tx has been built but broadcast has not yet been attempted
	defer func() {
		go func() {
			accessedAt := time.Now()
				...
				if contractAbi, ok := params["abi"]; ok {
					...
					if err == nil {
						...
						if name != "" && decimals != nil && symbol != "" { // isERC20Token
							...
							if token.Create() {
								common.Log.Debugf("Created token %s for associated %s contract: %s", token.ID, *network.Name, c.ID)
								ticker.Stop()
								return
							} else {
								common.Log.Warningf("Failed to create token for associated %s contract creation %s; %d errs: %s", *network.Name, c.ID, len(token.Errors), *common.StringOrNil(*token.Errors[0].Message))
							}
						}
					} else {
