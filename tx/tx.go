...
func (txs *TransactionSigner) GetSignerDetails() (address, derivationPath *string, err error) {
	...
	if txs.Wallet.Path != nil {
		...
		// DEV: should have address: ***"0x6Af845bae76F5cc16bC93F86b83E8928c3dfDa19"
		if err != nil {
	...
	if txs.Wallet.Path == nil {
		...
		//TODO this should return the derivation path used so we can reuse it to sign the transaction
		// lol, Kyle was right :)
		address = key.Address
...
func (t *Transaction) handleTxReceipt(
	...
	if t.To == nil {
		...
		if network.IsEthereumNetwork() {
			...
			common.Log.Debugf("deployed '%s' contract; address: %s", *contractAddress)
		}
