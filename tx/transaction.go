		...
		go func() {
			...
			for {
				...
				case <-ticker.C:
					...
					if err != nil {
						common.Log.Debugf("Failed to fetch ethereum tx receipt with tx hash: %s; %s", *t.Hash, err.Error())
						if err == ethereum.NotFound {
							if time.Now().Sub(startedAt) >= receiptTickerTimeout {
								common.Log.Warningf("Failed to fetch ethereum tx receipt with tx hash: %s; timing out after %v", *t.Hash, receiptTickerTimeout)
								t.updateStatus(db, "failed", common.StringOrNil("failed to fetch tx receipt"))
								ticker.Stop()
								return
							}
						} else {
							if time.Now().Sub(startedAt) >= receiptTickerTimeout {
								...
								t.updateStatus(db, "failed", common.StringOrNil(err.Error()))
								ticker.Stop()
								return
							}
						}
...
func (t *Transaction) handleEthereumTxReceipt(db *gorm.DB, network *Network, wallet *Wallet, receipt *types.Receipt) {
	...
	if t.To == nil {
		...
		if contract.Create() {
			common.Log.Debugf("Created contract %s for %s contract creation tx: %s", contract.ID, *network.Name, *t.Hash)
