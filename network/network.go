			...
			if accounts, accountsOk := chainspec["accounts"].(map[string]interface{}); accountsOk {
				...
				for _, addr := range addrs {
					...
					if constructorOk && abiOk {
						...
						// FIXME-- send to NATS
						common.Log.Debugf("Params: %s", params)
						...
						// if contract.Create() {
						// 	common.Log.Debugf("Created contract %s for %s network chainspec account: %s", contract.ID, *n.Name, addr)
						// } else {
						// 	common.Log.Warningf("Failed to create contract for %s network chainspec account: %s; %s", *n.Name, addr, *contract.Errors[0].Message)
						// }
					}
				}
			}
	...
	for {
		...
		case <-ticker.C:
			...
			if n.isEthereumNetwork() {
				...
				if node != nil && node.ID != uuid.Nil {
					if isLoadBalanced {
						common.Log.Warningf("Studio IDE load balancer may contain unhealthy or undeployed nodes")
					} else {
						if node.reachableOnPort(defaultWebappPort) {
