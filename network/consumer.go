					...
					if err != nil {
						...
						if txs, txsOk := result["transactions"].([]interface{}); txsOk {
							for _, _tx := range txs {
								...
								// 	common.Log.Warningf("Failed to set block and finalized_at timestamp on tx: %s", txHash)
								// 	continue
								// }
								...
								// }
								// if len(tx.Errors) > 0 {
								// 	common.Log.Warningf("Failed to set block and finalized_at timestamp on tx: %s; error: %s", txHash, tx.Errors[0].Message)
								// }
							}
						}
					}
...
func consumeLoadBalancerUnbalanceNodeMsg(msg *stan.Msg) {
	...
	db.Where("id = ?", networkNodeID).Find(&node)
	if node == nil || node.ID == uuid.Nil {
		common.Log.Warningf("Failed to removal network node from load balancer; no network node resolved for id: %s", networkNodeID)
		return
	}
