...
-e PGP_PRIVATE_KEY='-----BEGIN PGP PRIVATE KEY BLOCK-----
...
-e PGP_PASSPHRASE=password \
-e JWT_SIGNER_PUBLIC_KEY='-----BEGIN PUBLIC KEY-----
...
-----END PUBLIC KEY-----' \
-e CONSUME_NATS_STREAMING_SUBSCRIPTIONS=true \
-e NATS_CLUSTER_ID=provide \
-e NATS_TOKEN=testtoken \
-e NATS_URL=nats://docker.for.mac.localhost:4222 \
-e NATS_JETSTREAM_URL=nats://docker.for.mac.localhost:4222 \
-e NATS_FORCE_TLS=false \
-e GIN_MODE=release \
-e DATABASE_LOGGING=false \
-e DATABASE_USER=nchain \
-e DATABASE_PASSWORD=nchain \
