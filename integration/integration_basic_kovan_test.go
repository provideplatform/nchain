// +build integration nchain kovan basic
...
func _TestContractHDWalletKovanApp(t *testing.T) {
...
func TestContractAccountKovanApp(t *testing.T) {
	...
	tt := []struct {
		...
		{kovanNetworkID, "readwrite", rwCompiledArtifact},
	}
	...
	for _, tc := range tt {
		...
		t.Logf("creating contract using account id: %s, ref: %s", account.ID, contractRef)
		contract, err := nchain.CreateContract(*appToken.Token, map[string]interface{}{
			"network_id":     tc.network,
			"application_id": app.ID.String(),
		...
		msg := common.RandomString(118)
		t.Logf("msg: %s", msg)
		t.Logf("executing contract using account id: %s", account.ID.String())
		...
		case "ekho":
			...
			json.Unmarshal([]byte(parameter), &params)
		case "readwrite":
			parameter := fmt.Sprintf(`{"method":"setString", "params": ["%s"], "value":0, "account_id":"%s", "ref": "%s"}`, msg, account.ID, txRef)
			t.Logf("parameter is: %s", parameter)
			json.Unmarshal([]byte(parameter), &params)
		}
		...
		if tc.name == "readwrite" {
			...
			time.Sleep(10 * time.Second)
			params := map[string]interface{}{}
			parameter := fmt.Sprintf(`{"method":"getString", "params": [""], "value":0, "account_id":"%s"}`, account.ID.String())
			json.Unmarshal([]byte(parameter), &params)
