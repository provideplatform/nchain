...
func (t *Transaction) sign(db *gorm.DB, network *Network, wallet *Wallet) error {
	...
	if network.isEthereumNetwork() {
		...
		if wallet.PrivateKey != nil {
			...
			t.SignedTx, t.Hash, err = provide.EVMSignTx(network.ID.String(), network.rpcURL(), wallet.Address, _privateKey, t.To, t.Data, t.Value.BigInt(), wallet.TxCount(), uint64(gas))
		} else {
