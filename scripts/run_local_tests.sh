...
rm goldmine
go build .
NATS_TOKEN=testtoken NATS_URL=nats://localhost:4221 NATS_STREAMING_URL=nats://localhost:4222 NATS_CLUSTER_ID=provide NATS_STREAMING_CONCURRENCY=1 GIN_MODE=release DATABASE_NAME=${DB_NAME} DATABASE_USER=goldmine DATABASE_PASSWORD=goldmine DATABASE_HOST=localhost AMQP_URL=amqp://ticker:ticker@10.0.0.126 AMQP_EXCHANGE=ticker LOG_LEVEL=DEBUG /usr/local/bin/go test -timeout 30s -run \^\(TestGoldmine\)\$
#-run \^\(TestNetwork_Create\)\$
